...
def process_new_rows(csv_file, start_address, client_id, client_secret,
    ...
    if '우편번호' not in df_original.columns:
        raise ValueError("CSV 파일에 '우편번호' 컬럼이 없습니다. '우편번호' 컬럼이 필요합니다.")
    ...
    if os.path.exists(output_file):
        ...
        if '우편번호' not in df_result.columns:
            raise ValueError("결과 파일에 '우편번호' 컬럼이 없습니다.")
    ...
    # (3) 기존 결과의 우편번호 확인
    completed_zipcodes = set(df_result['우편번호'].tolist())

    # (4) 새로 계산해야 할 행만 필터링
    df_new = df_original[~df_original['우편번호'].isin(completed_zipcodes)]
    if len(df_new) == 0:
    ...
    for _, row in df_new.iterrows():
        count += 1
        address = row['주소']
        print(f"\n처리 중... 우편번호={row['우편번호']} [{count}/{len(df_new)}] : {address}")
        ...
        if count % 100 == 0:
            ...
            df_interim.to_csv(output_file, index=False, encoding='utf-8-sig')
