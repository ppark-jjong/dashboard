...
export class DashboardController {
  ...
  async createDashboard(dashboardData) {
    try {
      await this.apiService.createDashboard(dashboardData);
  ...
  async deleteDashboards() {
    try {
      const selectedDps = Array.from(this.state.selectedRows);
      if (selectedDps.length === 0) {
        this.view.showToast('알림', '삭제할 항목을 선택해주세요.');
        return;
      }

      if (!confirm(`선택한 ${selectedDps.length}개 항목을 삭제하시겠습니까?`)) {
        return;
      }

      await this.apiService.deleteDashboards(selectedDps);
      this.state.selectedRows.clear();
      this.view.clearSelection();
      await this.refreshData(true);
      this.view.showToast(
        '성공',
        `${selectedDps.length}개 항목이 삭제되었습니다.`
      );
    } catch (error) {
      this.handleError(error);
    }
