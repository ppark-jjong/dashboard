...
from backend.model.board_model import Driver
...
async def refresh_dashboard(
    ...
    try:
        ...
        return BaseResponse(success=True, message="데이터가 성공적으로 갱신되었습니다.")
...
@router.post(
    "/assignDriver", response_model=BaseResponse, dependencies=[Depends(require_token)]
)
async def assign_driver(
    ...
    try:
        ...
        if not request.dpsList:
            raise HTTPException(status_code=400, detail="배송 목록이 비어있습니다.")

        if request.driver_id <= 0:
            raise HTTPException(status_code=400, detail="유효하지 않은 기사 ID입니다.")
        ...
        raise HTTPException(
            status_code=500, detail=f"서버 처리 중 오류가 발생했습니다: {str(e)}"
        )


@router.put(
    "/dashboard/{dps}/status",
    response_model=BaseResponse,
    dependencies=[Depends(require_token)],
)
...
@router.get("/drivers", dependencies=[Depends(require_token)])
async def get_drivers(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="기사 목록을 불러오는 중 오류가 발생했습니다."
        )
